...
function validate(): boolean {
  ...
  if (!username.value.trim()) next.username = '请输入管理员用户名'
    ...
    <div class="w-full max-w-sm z-10">
      <div class="flex flex-col gap-6">
        <div class="flex flex-col items-center gap-2 text-center">
          ...
          <p class="text-xs text-muted-foreground font-medium">多维实时监控与运行态势分析系统</p>
        </div>
        ...
        <Card class="border-border/80 shadow-sm bg-card rounded-xl">
          <CardHeader class="pb-4">
            ...
            <CardDescription class="text-xs text-muted-foreground font-normal">输入系统凭据以进入监控大屏</CardDescription>
          </CardHeader>
          <CardContent>
            <form @submit.prevent="submit">
              <FieldGroup class="gap-4">
                ...
                <div class="pt-2">
                  <LoadingButton type="submit" class="w-full font-semibold h-9.5 text-sm cursor-pointer shadow-xs" :loading="loading" :disabled="!canSubmit">
                    立即登录
                  </LoadingButton>
                </div>
              </FieldGroup>
